...
// Set working hours (barber only)
router.post('/hours', authenticateToken, authorizeRoles('barber'), async (req, res) => {
  const { hours } = req.body;
  const userId = req.user.id;
  ...
  try {
    ...
    for (const hour of hours) {
      await pool.query(
        'INSERT INTO working_hours (shop_id, day_of_week, start_hour, end_hour) VALUES ($1, $2, $3, $4)',
        [shopId, hour.day_of_week, hour.start_hour, hour.end_hour]
      );
    }
...
// Get my shop hours (barber only)
router.get('/hours/my-hours', authenticateToken, authorizeRoles('barber'), async (req, res) => {
  const userId = req.user.id;
  try {
    const shopResult = await pool.query('SELECT id FROM barber_shops WHERE owner_id = $1', [userId]);
    if (shopResult.rows.length === 0) {
    ...
    const result = await pool.query('SELECT day_of_week, start_hour, end_hour FROM working_hours WHERE shop_id = $1 ORDER BY day_of_week', [shopResult.rows[0].id]);
    res.json(result.rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
